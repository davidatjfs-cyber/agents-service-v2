...
/** 本地 Ollama（如 gemma4:31b），不依赖 DashScope；失败时由上层回退到 API */
async function callOllamaLLM(messages, options = {}) {
  const base = String(process.env.OLLAMA_BASE_URL || 'http://127.0.0.1:11434').replace(/\/$/, '');
  const model = String(process.env.OLLAMA_CHAT_MODEL || process.env.OLLAMA_OPERATIONS_MODEL || 'gemma4:31b').trim();
...
export async function callLLM(messages, options = {}) {
  ...
  if (!hasTools && routedModel === 'gemma4:31b') {
    const o = await callOllamaLLM(messages, options);
    if (o.ok && o.content) return o;
    logger.warn({ err: o.error }, 'Ollama (gemma4:31b) failed, falling back to API LLM');
  }

  let primaryModel = options.model;
  if (!primaryModel) {
    if (!routerCtx || routedModel === 'gemma4:31b') {
