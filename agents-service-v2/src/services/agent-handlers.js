...
import { saveMemory, recallMemories, getOutcomeStats, buildMemoryContextBlock } from './agent-memory.js';
import {
  writeWikiKnowledge,
...
async function handleDataAuditor(text, ctx) {
  ...
  if (!ds) ds = '\n[no data found]\n';
  // P2: 记忆回调
  const daMemoryBlock = store ? await buildMemoryContextBlock('data_auditor', store, text).catch(() => '') : ''; if (daMemoryBlock) ds += daMemoryBlock;
  if (mode === 'decision') {
    try {
...
`;
  ...
  // P1: Agent互助 — 差评分析后附带培训建议（等待结果，追加到回复）
  if (store && /差评|投诉|服务.*问题|出品.*问题|bad_review/.test(text)) {
    try {
      const trainingResult = await requestAgent('data_auditor', 'train_advisor', '基于以下差评分析，给出培训建议：\n' + (cleanedAns || text).slice(0, 500), { store }, 10000);
      if (trainingResult.ok && trainingResult.response) {
        cleanedAns += '\n\n---\n🎓 **培训建议（系统自动生成）**\n' + trainingResult.response.slice(0, 400);
      }
    } catch (e) { /* fail-soft */ }
  }
  try {
...
async function handleOpsSupervisor(text, ctx) {
  ...
  const opsMemoryBlock = store ? await buildMemoryContextBlock('ops_supervisor', store, text).catch(() => '') : '';
  const brand = store ? await getBrandForStore(store).catch(() => null) : null;
  const tr = extractTimeRangeFromText(text);
  const timeLabel = getTimeLabelChinese(tr);
...
`;
  ...
  // P1: Agent互助 — 巡检涉及培训问题时附带SOP参考
  let sopSupplement = '';
  if (store && /SOP|标准|流程|培训|规范|操作/.test(text)) {
    const sopResult = await requestAgent('ops_supervisor', 'train_advisor', '请提供与门店巡检相关的最新SOP标准：' + text, { store }).catch(() => ({ ok: false }));
    if (sopResult.ok && sopResult.response) sopSupplement = '\n\n---\n📋 **SOP参考（系统自动查询）**\n' + sopResult.response.slice(0, 300);
  }
  const reportTitle = /开档|谁没|提交情况/.test(text) ? '开档提交情况' : /收档|闭市/.test(text) ? '收档提交情况' : '营运巡检';
  const finalResponse = (r.content || '请描述巡检需求。') + sopSupplement;
  return { agent: 'ops_supervisor', response: finalResponse, store, data: opsData, timeLabel, reportTitle };
}
...
async function handleChiefEvaluator(text, ctx) {
  ...
  const ceMemoryBlock = store ? await buildMemoryContextBlock('chief_evaluator', store, text).catch(() => '') : '';
  const brand = store ? await getBrandForStore(store).catch(() => null) : null;
  if (store) {
    try {
...
async function handleTrainAdvisor(text, ctx) {
  const store = ctx.store || '', user = ctx.username || '';
  const taMemoryBlock = store ? await buildMemoryContextBlock('train_advisor', store, text).catch(() => '') : '';
  const brand = store ? await getBrandForStore(store).catch(() => null) : null;
...
async function handleAppeal(text, ctx) {
  let appealData = '';
  const store = ctx.store || '', user = ctx.username || '';
  const appealMemoryBlock = store ? await buildMemoryContextBlock('appeal', store, text).catch(() => '') : '';
  const brand = store ? await getBrandForStore(store).catch(() => null) : null;
  try {
...
async function handleMarketingPlanner(text, ctx) {
  ...
  let mktData = '';
  const store = ctx.store || '';
  const mplMemoryBlock = store ? await buildMemoryContextBlock('marketing_planner', store, text).catch(() => '') : '';
  const brand = store ? await getBrandForStore(store).catch(() => null) : null;
  const data = {};
...
${unifiedKnowledgeBlock_mkt}${mplMemoryBlock || ''}
...
【渠道风格指南】
- 朋友圈：短平快、带情绪、多用感叹号和表情、发券时写「福利」而非「优惠」
- 企微：专业简洁、用「您」称呼、适合发券 + 菜品推荐组合、结尾留「回复可咨询」
- 小红书：轻松口语化、多用「谁懂啊」「绝了」「被...到了」、发菜品图比文案重要
- 抖音：短视频风格、短句+高频节奏、用「家人们」「姐妹们」称呼、突出限时和互动
- 短信：极简、30字以内表达核心利益点、必须含「回T退订」
- 大众点评：客观评论风格、突出性价比和环境、适合套餐推荐而不是单品券
 5. 语气专业、可交给门店直接执行
...
async function handleMarketingExecutor(text, ctx) {
  let execData = '';
  const store = ctx.store || '';
  const execMemoryBlock = store ? await buildMemoryContextBlock('marketing_executor', store, text).catch(() => '') : '';
  const brand = store ? await getBrandForStore(store).catch(() => null) : null;
...
${unifiedKnowledgeBlock_exec}${execMemoryBlock || ''}
【如何撰写每个部分】
...
async function handleMaster(t, c) {
  const store = c.store || '';
  const masterMemoryBlock = store ? await buildMemoryContextBlock('master', store, t).catch(() => '') : '';
  const brand = store ? await getBrandForStore(store).catch(() => null) : null;
  const memories = [];
  try {
...
【输出约束】
${unifiedKnowledgeBlock_master}${masterMemoryBlock || ''}
...
/**
 * 复合路由处理 — 串行调用多个Agent，合并结果
 */
async function handleComposite(text, ctx) {
  const routes = ctx.routes || [];
  if (!routes.length) {
    return { agent: 'composite', response: '无法确定需要哪些专业协助', store: ctx.store || '' };
  }
  const results = [];
  for (const route of routes) {
    try {
      const r = await dispatchToAgent(route, text, { ...ctx, isSubRequest: true });
      if (r && r.response) results.push(r);
    } catch (e) {
      logger.warn({ err: e?.message, route }, 'composite sub-agent failed');
    }
  }
  const combined = results.map(r => r.response).filter(Boolean).join('\n\n');
  return {
    agent: 'composite',
    response: combined || '暂时无法获取相关信息',
    data: results.map(r => r.data || '').filter(Boolean).join('\n'),
    store: ctx.store || '',
  };
}
