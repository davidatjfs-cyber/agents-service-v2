/**
 * ════════════════════════════════════════════════════════════════════════════════
 * ！严格警告：此文件直接影响员工绩效工资，修改前必须管理员确认
 * ════════════════════════════════════════════════════════════════════════════════
 *
 * 核心业务规则（不可违反）：
 *
 *   1. 绩效分数当月内必须累计统计
 *      - total_score = 100 - 本月累计扣分（先查上一周同月分数算出已扣分）
 *      - 分数可以是负数（扣分超过100分时）
 *      - 跨月才可以重新从100开始计算
 *
 *   2. 周度统计遇到跨月（一周跨两个自然月），必须拆成上月和本月两个统计
 *      - 上月段 period = week_YYYYMMDD__YYYYMM（上月）
 *      - 本月段 period = week_YYYYMMDD__YYYYMM（本月）
 *      - 上月段累计到上月的扣分，本月段从100开始
 *      - 下周一评分时，必须正确查到本月的上一周分数（含跨月拆分记录）
 *
 *   3. 每月第一周评分时，找出同月内所有之前的分数记录（含跨月拆分的 __YYYYMM 后缀记录），
 *      计算 priorMonthDeducted = 100 - 上一周total_score，新分数 = priorMonthDeducted - 本周扣分
 *
 *   4. 修改此文件后必须手动执行一次 runWeeklyStoreScoring() 验证分数正确
 *
 * ！错误修改会导致全员绩效分错误，后果极其严重
 * ！git CODEOWNERS 已配置此文件必须管理员审批
 * ════════════════════════════════════════════════════════════════════════════════
 *
