...
【平台分发与流量友好（对齐公开规则里的「可优化结构」，禁止玄学承诺）】
- 共性：每条须便于系统理解主题与受众——标题/首句含「品类+场景或利益点」之一；正文信息密度适中，避免关键词无意义重复（易被判定低质或搬运）；不蹭无关热点、不标题党到文不对题（降权风险）。
- 大众点评：偏「决策参考」——写清适合谁、何时来、和同类相比的差异点；鼓励真实细节与可读结构（分段），利于精选与搜索曝光；避免违规诱导好评话术。
- 小红书：偏「搜索+推荐」——标题覆盖 1～2 个用户可能搜索的词根，正文前 80 字内出现核心词；话题标签兼顾品类词与场景词，且与正文强相关（乱标无关话题不利于推荐）。
- 抖音：偏「完播与互动信号」——前 3 秒信息冲突或悬念；中段节奏快、少废话；结尾互动题要「一秒能答」；口播与画面建议一致，避免空洞配乐旁白。
- 外卖：偏「转化与复购」——清晰品类、规格、适合时段、搭配建议；可写搜索词联想但不堆砌；不写虚假折扣。

【流量获取总结参照表——写每一段前先在脑子里对齐，禁止把表格原样复制到输出里】
| 平台 | 核心策略 | 流量侧重点（结构对齐，非玄学） | 建议配图/视频方向 |
| 小红书 | 种草、情绪与生活场景 | 真实分享感、可搜词+首段留存、封面大字提示；用具体画面写「好看/舒服」而非空喊「氛围感」三个字 | 高清、自然光或暖光、生活化场景，主体清晰，少过度摆拍广告脸 |
| 大众点评 | 实用、信任与决策 | 细节可核对、适合谁/何时来、与同类差异；客观口吻但不冷冰冰 | 菜品特写、环境一角、门头桌景；禁止虚构评分截图 |
| 抖音 | 视觉冲击、快节奏 | 前 3 秒钩子、口语短句、强节奏；可提示 BGM 气质（轻快/市井/馋感）与剪辑节奏，不写「必上热门」 | 特写、慢动作可点缀、快切口播；字幕与嘴型节奏一致 |
| 外卖 | 清晰与转化 | 到手场景、规格、搭配、搜索联想词；一句解决「现在点不踩雷」 | 餐盒拆开、汤汁分离、到手温度感描写（文字侧） |
...
【去 AI 味——最高优先级；出现任一风格或词汇即算失败，整段重写】
禁用词与禁用腔（含变体、同义堆叠）：综上所述、值得一提、不难发现、不仅…而且…更…、在当下的、在…的背景下、无论是…还是…、深度、赋能、痛点、用户、极致体验、不容错过、宝藏、绝绝子、YYDS、姐妹们谁懂、家人们、沉浸式、氛围感拉满、一口沦陷、好吃到哭、为您呈现、致力于、全方位、多元化、显著提升、不容错过的一次体验。
写作要像真人打字：多用短句与口语停顿，可适度省略主语；禁止排比三板斧、禁止段尾三个「让…」连用；禁止连续三个以上四字成语或空洞形容词并列。
禁止「小作文」起承转合模板（先戴帽再分点再喊口号）；信息直接落在场景与细节上。
...
【反同质化——否则视为不合格】
- 同一轮输出的 ${sectionCount} 段，须像「不同小编各写一条」：叙事人称（我们店/后厨视角/老客口吻/店长碎碎念等）在相邻两段不得相同；禁止共用同一句式骨架（如都以「如果你也…」开头）。
- 同一平台若有多套（第1套/第2套）：第二套须换「叙事壳」——从下列至少换一类：探店故事线 / 与老客对话 / 厨师小课堂 / 价格透明梗 / 场景纠错（别点错吃法）/ 限时场景（雨天/加班夜）；两套之间禁止只替换 2～3 个名词。
- 全轮禁止复用同一比喻、同一谐音梗、同一句「金句」；禁止四条结尾都用同一种号召句式。
...
async function runGeneration(sess) {
  ...
  const r = await callLLM(
    [
      ...
      {
        role: 'user',
        content: `菜品与品牌信息：\n${brief}\n\n图片要点：\n${visualNotes}\n\n请严格按 ${sectionCount} 个标题依次输出；每条须符合对应平台的体裁、流量结构与分发友好度（点评堂食、外卖到家、小红书笔记、抖音短视频脚本），禁止漏块、禁止跨平台混用场景与话术。\n刻意拉开各段语气与叙事壳，避免整轮读起来像同一人用同一模板刷了四遍。`
      }
    ],
    {
