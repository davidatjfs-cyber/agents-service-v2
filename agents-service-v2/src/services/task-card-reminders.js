...
async function recordStandardChaseAttitudeOnly(task) {
  ...
  if (chatId) {
    await sendGroup(
      chatId,
      `【HR/总部备案·工作态度】门店 ${task.store} 任务 ${task.task_id}（${task.source}）三次催办后仍未有效闭环，已打标「工作态度未完成」备案（计入月度态度统计；不写入 agent_scores 扣分）。`
    ).catch(() => {});
  }
  ...
  logger.info({ taskId: task.task_id, source: task.source, code }, 'task-reminder: chase → attitude only');
  return true;
}
