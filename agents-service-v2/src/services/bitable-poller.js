...
只输出回复内容，不要任何前缀或说明。`;
    ...
    logger.debug({ err: e?.message }, 'bad_review: ai reply generation skipped');
  }

  let card;
  try {
    card = buildBadReviewCard({ store, date, platform, rating, responsibility, content, weekCount, monthCount, aiReply });
  } catch (e) {
